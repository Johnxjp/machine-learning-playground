...
class VAEEncoder(nn.Module):
    """
    Variational encoder that compresses 28×28 MNIST images to latent distribution parameters.
    ...
    Architecture:
        ...
        - ReLU + BatchNorm2d
        - Flatten: 4×4×64 = 1024
        - Linear: 1024 → latent_dim (for mu)
        - Linear: 1024 → latent_dim (for logvar)
    """
...
class VAEDecoder(nn.Module):
    ...
    Architecture (same as standard autoencoder):
...
class VAE(nn.Module):
    ...
    Compresses 28×28 grayscale images to a probabilistic latent space and reconstructs them.
    Uses the reparameterization trick for training with backpropagation.
    """
    ...
    def reparameterize(self, mu: torch.Tensor, logvar: torch.Tensor) -> torch.Tensor:
        """
        Reparameterization trick: z = mu + eps * std, where eps ~ N(0, 1).
    ...
    def sample(self, num_samples: int, device: str = 'cpu') -> torch.Tensor:
        """
        Sample images from the prior distribution N(0, I).
